...
#define MYMACLOG // khe: undef to use orignial version
...
MmWavePhyRxTrace::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MmWavePhyRxTrace")
    ...
    .AddAttribute ("RxPacketTraceFilename",
                   "Name of the file where the received results will be saved.",
                   StringValue ("RxPacketTrace.txt"),
                   MakeStringAccessor (&MmWavePhyRxTrace::SetRxPacketTraceFilename),
                   MakeStringChecker ())
  ;
  return tid;
}
...
*/
void
MmWavePhyRxTrace::SetRxPacketTraceFilename (std::string outputFilename)
{
   m_rxPacketTraceFilename = outputFilename;
}
...
MmWavePhyRxTrace::RxPacketTraceUeCallback (Ptr<MmWavePhyRxTrace> phyStats, std::string path, RxPacketTraceParams params)
{
	if (!m_rxPacketTraceFile.is_open())
	{
		m_rxPacketTraceFile.open(m_rxPacketTraceFilename.c_str (), std::ofstream::out | std::ofstream::app);
		if (!m_rxPacketTraceFile.is_open())
		{
...
#else
	if (!params.m_corrupt){
		m_rxPacketTraceFile << params.m_frameNum << "." << (unsigned)params.m_sfNum << ","
			<< params.m_tbSize << std::endl;
	} else {
		NS_LOG_DEBUG ("DL TB error\t" << params.m_frameNum << "\t" << (unsigned)params.m_sfNum << "\t" << (unsigned)params.m_symStart
		    << "\t" << (unsigned)params.m_numSym
				<< "\t" << params.m_rnti << "\t" << params.m_tbSize << "\t" << (unsigned)params.m_mcs << "\t" << (unsigned)params.m_rv << "\t"
...
#endif
	//m_rxPacketTraceFile.close (); // khe: just to make sure, that a new file will be opened
}
void
