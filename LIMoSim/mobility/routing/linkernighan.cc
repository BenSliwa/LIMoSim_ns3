...
LinKernighan::LinKernighan(vector<vector<double> > &_edgeDistances, vector<uint> &ids) {
    this->edgeDistances = _edgeDistances;
    this->ids = ids;
    size = ids.size();

    // initialize tour
    tour = vector<uint>(size, 0);
...
vector<uint> LinKernighan::getCurrentTour()
{
    std::vector<uint> result (tour.size());
    uint count = 0;
    uint start = tour[0];
    while (start != 0) {
        result.at(count) = start;
        start = tour[start];
        count++;
    }
    return result;
...
double LinKernighan::getCurrentTourDistance() {
    int currentIndex = 0;
    double distance = 0;
    for (int i = 0; i < size; i++) {
...
void LinKernighan::LKMove(int tourStart) {
    set<pair<int,int> > broken_set, joined_set;
    vector<uint> tour_opt = tour;
    double g_opt = 0;
    double g = 0; // := G_i
    double g_local; // := g_i
...
void LinKernighan::optimizeTour() {
 ...
 */
void LinKernighan::reverse(int start, int end) {
    int current = start;
    int next = tour[start];
    int nextNext;
    do {
...
void LinKernighan::printTourIds() {
    int current = 0;
    do {
